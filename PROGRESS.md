...
## Completed Tasks
- [x] Release v1.2.0-beta2: In-Order Queue Synchronization & Redundant Wait Removal (2026-08-27): Configured SYCL queues across `test/main.cpp` and `test/test_self_knn.cpp` to use `sycl::property::queue::in_order{}`. Audited all synchronization barriers across `src/hlbvh.hpp` and `src/domain_decomposition.hpp`, removing redundant `.wait()` calls between intermediate kernel submissions (e.g. `q.fill`, `sfc_encode`, `oneapi::dpl::sort`, binary searches, packing kernels) while preserving strictly necessary synchronization barriers immediately preceding host reads, MPI operations, and `sycl::free()` deallocations.
