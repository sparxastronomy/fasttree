...
**Current release version:** v.1.1.0 (Release Candidate)
**Target release version:** v.1.2.0 (Planned)

## Completed Tasks
- [x] Release v1.2.0: Scaling Benchmark Multi-Precision Compatibility & Automation (2026-08-07): Refactored `test/benchmark/` code (`BenchmarkUtils.hpp`, `knn_query_scaling.cpp`, `range_query_scaling.cpp`, `gpu_sort_scaling.cpp`, `tree_build_scaling.cpp`, `rebuild_scaling.cpp`, `sfc_encoding_scaling.cpp`) to support both double-precision floating-point and integer coordinate representations (`int32`, `int64`). Updated `knn_query_scaling.cpp` results buffer pointer to `size_t*`. Added `-Wno-c2y-extensions` flag to `test/benchmark/CMakeLists.txt` for `icpx` under C++23. Created automated scaling test execution script `test/benchmark/run_cpu_scaling.sh` covering 6 CPU configurations (`double`, `int32`, `int64` with `PERIODIC_BC=OFF/ON`) outputting Markdown results to `docs/benchmark_results/`.
