...
int main(int argc, char **argv) {
  ...
  while (std::getline(config, line)) {
    ...
    constexpr int TOP_BITS = 24;
    {
      ...
      particles<coord_t> redistributed_p = redistribute_particles(q, p, splitters, bbox, TOP_BITS);
#endif
    ...
    for (int iter = 0; iter < num_iterations; ++iter) {
      ...
      if (h_max <= static_cast<coord_t>(0.0)) h_max = static_cast<coord_t>(2.0);  // safety fallback
    ...
    if (rank == 0) {
      ...
      std::cout << "----------------------------------------------------------------------------------------\n";
      std::cout << std::left << std::setw(30) << "Phase" << std::right << std::setw(15) << "Min Time (s)" << std::setw(15) << "Max Time (s)"
                << std::setw(15) << "Avg Time (s)" << "\n";
      std::cout << "----------------------------------------------------------------------------------------\n";

      const char *phase_names[7] = {"1. Bounding Box",           "2. Coarse Histogram",   "3. Splitter Gen",           "4. Local Binning & Redist",
                                    "5. Explicit Halo Exchange", "6. Unified Tree Build", "Total Domain Decomposition"};

      for (int i = 0; i < 7; ++i) {
        std::cout << std::left << std::setw(30) << phase_names[i] << std::right << std::fixed << std::setprecision(6) << std::setw(15) << min_times[i]
                  << std::setw(15) << max_times[i] << std::setw(15) << (sum_times[i] / size) << "\n";
      }
      std::cout << "----------------------------------------------------------------------------------------\n";
      std::cout << "Peak RSS Memory per Rank:  Max: " << std::fixed << std::setprecision(2) << max_rss << " MB, Avg: " << (sum_rss / size) << " MB\n";
      std::cout << "========================================================================================\n\n";
    }
  }
