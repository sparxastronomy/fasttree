...
bool load_hdf5_data_slice(const std::string& filename, size_t offset, size_t count, ParticleData& data) {
    ...
    if (dset_coords >= 0) {
        ...
        if (H5Dread(dset_coords, H5T_NATIVE_FLOAT, memspace, filespace, H5P_DEFAULT, coords.data()) >= 0) {
    ...
    if (dset_vels >= 0) {
        ...
        if (H5Dread(dset_vels, H5T_NATIVE_FLOAT, memspace, filespace, H5P_DEFAULT, vels.data()) >= 0) {
            for (size_t i = 0; i < count; ++i) {
...
int main(int argc, char** argv) {
    ...
    if (!config.is_open()) {
        if (rank == 0) std::cerr << "Error: Could not open config file " << config_path << std::endl;
        MPI_Finalize();
        return 1;
    }
    ...
    while (std::getline(config, line)) {
        ...
        for (size_t i = 0; i < num_files; ++i) {
            ...
            if (intersect_start < intersect_end) {
                ...
                if (load_hdf5_data_slice(file_paths[i], read_offset, read_count, temp_data)) {
                    hdf5_data.pos_x.insert(hdf5_data.pos_x.end(), temp_data.pos_x.begin(), temp_data.pos_x.end());
                    hdf5_data.pos_y.insert(hdf5_data.pos_y.end(), temp_data.pos_y.begin(), temp_data.pos_y.end());
                    hdf5_data.pos_z.insert(hdf5_data.pos_z.end(), temp_data.pos_z.begin(), temp_data.pos_z.end());
        ...
        particles<float> p_orig;
        p_orig.pos_x = hdf5_data.pos_x;
        p_orig.pos_y = hdf5_data.pos_y;
        p_orig.pos_z = hdf5_data.pos_z;
        p_orig.mass = hdf5_data.masses;
        ...
        // Run warm up
        {
            particles<float> p = p_orig;
            BoundingBox bbox = get_global_bounding_box(q, p);
            std::vector<int> hist = get_global_histogram(q, p, bbox, 20);
            std::vector<uint32_t> splitters = generate_splitters(hist, size, 20);
            particles<float> redistributed_p = redistribute_particles(q, p, splitters, bbox, 20);
            float h_max = 2.0f;
        ...
        double times_redist[3] = {0.0}, times_halo[3] = {0.0}, times_tree[3] = {0.0}, times_total[3] = {0.0};
        ...
        for (int iter = 0; iter < num_iterations; ++iter) {
            ...
            float h_max = 0.02f * (bbox.max_x - bbox.min_x);
            if (h_max <= 0.0f) h_max = 2.0f; // safety fallback
        ...
        double local_avg_redist = 0.0, local_avg_halo = 0.0, local_avg_tree = 0.0, local_avg_total = 0.0;
        ...
        double avg_times[7] = { local_avg_bbox, local_avg_hist, local_avg_splitters, local_avg_redist, local_avg_halo, local_avg_tree, local_avg_total };
        ...
        if (rank == 0) {
            std::cout << "Dataset: " << label << " (" << total_n << " particles in " << num_files << " file(s))\n";
            std::cout << "----------------------------------------------------------------------------------------\n";
            std::cout << std::left << std::setw(30) << "Phase" 
                      << std::right << std::setw(15) << "Min Time (s)" 
            ...
            for (int i = 0; i < 7; ++i) {
                std::cout << std::left << std::setw(30) << phase_names[i]
                          ...
                          << std::setw(15) << min_times[i]
                          << std::setw(15) << max_times[i]
                          << std::setw(15) << (sum_times[i] / size) << "\n";
            }
            std::cout << "----------------------------------------------------------------------------------------\n";
            std::cout << "Peak RSS Memory per Rank:  Max: " << std::fixed << std::setprecision(2) << max_rss 
                      << " MB, Avg: " << (sum_rss / size) << " MB\n";
            std::cout << "========================================================================================\n\n";
        }
    }
