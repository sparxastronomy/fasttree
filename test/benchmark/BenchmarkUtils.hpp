...
struct Timer {
  ...
  Timer(const std::string &n) : name(n) { start_time = std::chrono::high_resolution_clock::now(); }
...
bool load_hdf5_data(const std::string &filename, ParticleData &data) {
  ...
  // Load Masses (N)
  if (H5LTread_dataset_float(group_id, "Masses", data.masses.data()) < 0) { std::cerr << "Error: Could not read Masses" << std::endl; }
...
void print_benchmark_row(const std::string &test_name, const std::string &hardware, size_t count, const std::string &params, double time_sec,
                         size_t memory_kb, const std::string &notes = "-") {
  std::cout << "| " << std::left << std::setw(25) << test_name << " | " << std::setw(20) << hardware << " | " << std::setw(15) << count << " | "
            << std::setw(25) << params << " | " << std::fixed << std::setprecision(6) << std::setw(12) << time_sec << " | " << std::fixed
            << std::setprecision(2) << std::setw(10) << (memory_kb / 1024.0) << " | " << std::setw(20) << notes << " |" << std::endl;
}
