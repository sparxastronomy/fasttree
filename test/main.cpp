...
static double int_dist_sq_to_phys(dist_t stored, double box_size) {
  ...
  constexpr double int_to_norm = 1.0 / static_cast<double>(1ULL << (BITS_PER_DIMENSION - _DIST_SHIFT));
...
int main() {
...
#if !defined(FASTTREE_INTEGER_COORDS)
  bbox = BoundingBox<coord_t>(static_cast<coord_t>(box_min), static_cast<coord_t>(box_max), static_cast<coord_t>(box_min),
                              static_cast<coord_t>(box_max), static_cast<coord_t>(box_min), static_cast<coord_t>(box_max));
  printf("Bounding Box: [%.3f, %.3f] x [%.3f, %.3f] x [%.3f, %.3f]\n", (double)bbox.min_x, (double)bbox.max_x, (double)bbox.min_y, (double)bbox.max_y,
         (double)bbox.min_z, (double)bbox.max_z);
#else
    ...
    for (int k : {5, 48}) {
      ...
      for (int i = 0; i < k; ++i) {
        ...
        if (std::abs(phys_d2 - bf_d2) > tol) {
          std::cout << "    FAILURE at rank " << i << ": got d2=" << phys_d2 << " expected d2=" << bf_d2 << "\n";
          knn_ok = false;
        }
  ...
  // ================================================================
  {
    ...
    for (int i = 0; i < k; ++i) {
      ...
      bool is_wrapped =
          (std::abs(px - float_qx) > 0.5 * box_size || std::abs(py - float_qy) > 0.5 * box_size || std::abs(pz - float_qz) > 0.5 * box_size);

      std::cout << "    [" << i << "] id=" << pid << " pos=(" << px << "," << py << "," << pz << ")"
                << " dist=" << std::sqrt(phys_d2) << (is_wrapped ? "  <-- periodic image" : "") << "\n";

      if (std::abs(phys_d2 - bf_d2) > tol) {
        std::cout << "    FAILURE at rank " << i << ": got d2=" << phys_d2 << " bf d2=" << bf_d2 << "\n";
        knn_ok = false;
      }
    }
