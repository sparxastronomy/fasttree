...
struct uint128_t {
  ...
  constexpr uint128_t operator^(const uint128_t &o) const noexcept { return uint128_t(hi ^ o.hi, lo ^ o.lo); }
...
struct sfc_key {
  ...
  constexpr bool operator==(const sfc_key &o) const noexcept { return hs == o.hs && is == o.is && ls == o.ls; }
...
static constexpr uint8_t rottable3[48][8] = {
    {36, 28, 25, 27, 10, 10, 25, 27}, {29, 11, 24, 24, 37, 11, 26, 26}, {8, 8, 25, 27, 30, 38, 25, 27},   {9, 39, 24, 24, 9, 31, 26, 26},
    {40, 24, 44, 32, 40, 6, 44, 6},   {25, 7, 33, 7, 41, 41, 45, 45},   {4, 42, 4, 46, 26, 42, 34, 46},   {43, 43, 47, 47, 5, 27, 5, 35},
    {33, 35, 36, 28, 33, 35, 2, 2},   {32, 32, 29, 3, 34, 34, 37, 3},   {33, 35, 0, 0, 33, 35, 30, 38},   {32, 32, 1, 39, 34, 34, 1, 31},
    {24, 42, 32, 46, 14, 42, 14, 46}, {43, 43, 47, 47, 25, 15, 33, 15}, {40, 12, 44, 12, 40, 26, 44, 34}, {13, 27, 13, 35, 41, 41, 45, 45},
    {28, 41, 28, 22, 38, 43, 38, 22}, {42, 40, 23, 23, 29, 39, 29, 39}, {41, 36, 20, 36, 43, 30, 20, 30}, {37, 31, 37, 31, 42, 40, 21, 21},
    {28, 18, 28, 45, 38, 18, 38, 47}, {19, 19, 46, 44, 29, 39, 29, 39}, {16, 36, 45, 36, 16, 30, 47, 30}, {37, 31, 37, 31, 17, 17, 46, 44},
    {12, 4, 1, 3, 34, 34, 1, 3},      {5, 35, 0, 0, 13, 35, 2, 2},      {32, 32, 1, 3, 6, 14, 1, 3},      {33, 15, 0, 0, 33, 7, 2, 2},
    {16, 0, 20, 8, 16, 30, 20, 30},   {1, 31, 9, 31, 17, 17, 21, 21},   {28, 18, 28, 22, 2, 18, 10, 22},  {19, 19, 23, 23, 29, 3, 29, 11},
    {9, 11, 12, 4, 9, 11, 26, 26},    {8, 8, 5, 27, 10, 10, 13, 27},    {9, 11, 24, 24, 9, 11, 6, 14},    {8, 8, 25, 15, 10, 10, 25, 7},
    {0, 18, 8, 22, 38, 18, 38, 22},   {19, 19, 23, 23, 1, 39, 9, 39},   {16, 36, 20, 36, 16, 2, 20, 10},  {37, 3, 37, 11, 17, 17, 21, 21},
    {4, 17, 4, 46, 14, 19, 14, 46},   {18, 16, 47, 47, 5, 15, 5, 15},   {17, 12, 44, 12, 19, 6, 44, 6},   {13, 7, 13, 7, 18, 16, 45, 45},
    {4, 42, 4, 21, 14, 42, 14, 23},   {43, 43, 22, 20, 5, 15, 5, 15},   {40, 12, 21, 12, 40, 6, 23, 6},   {13, 7, 13, 7, 41, 41, 22, 20}};
...
inline uint64_t top3_to_bottom(uint32_t w) noexcept { return w >> 29; }
...
inline uint64_t convert_to_sfc1d_impl(double d, uint64_t * /*tag*/) noexcept {
  ...
  constexpr int b_dim = (BITS_PER_DIMENSION < 52) ? BITS_PER_DIMENSION : 52;
  constexpr int shift = 52 - b_dim;
  constexpr uint64_t mask = (BITS_PER_DIMENSION < 64) ? ((uint64_t(1) << BITS_PER_DIMENSION) - 1u) : ~uint64_t(0);
...
inline uint32_t convert_to_sfc1d_impl(float f, uint32_t *tag) noexcept { return convert_to_sfc1d_impl(static_cast<double>(f), tag); }
inline uint64_t convert_to_sfc1d_impl(float f, uint64_t *tag) noexcept { return convert_to_sfc1d_impl(static_cast<double>(f), tag); }
inline uint128_t convert_to_sfc1d_impl(float f, uint128_t *tag) noexcept { return convert_to_sfc1d_impl(static_cast<double>(f), tag); }
...
using SortStrategy = std::conditional_t<KEY_TOTAL_BITS <= 64, SortSingle, std::conditional_t<KEY_TOTAL_BITS <= 128, SortDouble, SortTriple>>;
...
inline void print_sfc_config() {
  ...
  const char *sort_strat = KEY_TOTAL_BITS <= 64 ? "single-pass uint64_t radix" : KEY_TOTAL_BITS <= 128 ? "2-pass radix" : "3-pass radix";
  ...
  printf("  Grid resolution:    2^%d = ~%.2e cells/axis\n", BITS_PER_DIMENSION, std::pow(2.0, (double)BITS_PER_DIMENSION));
