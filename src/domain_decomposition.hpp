...
inline BoundingBox<FloatT> get_global_bounding_box(sycl::queue &q, const particles<FloatT> &p) {
    ...
    if constexpr (std::is_same_v<FloatT, uint128_t>) {
        return (n > 0) ? compute_bbox(q, p, n) : BoundingBox<FloatT>{};
    }
    ...
    if (n == 0) {
        ...
        MPI_Allreduce(
            &local_min_x, &global_min_x, 1, mpi_type_traits<FloatT>::type(), MPI_MIN, MPI_COMM_WORLD
        );
        MPI_Allreduce(
            &local_max_x, &global_max_x, 1, mpi_type_traits<FloatT>::type(), MPI_MAX, MPI_COMM_WORLD
        );
        MPI_Allreduce(
            &local_min_y, &global_min_y, 1, mpi_type_traits<FloatT>::type(), MPI_MIN, MPI_COMM_WORLD
        );
        MPI_Allreduce(
            &local_max_y, &global_max_y, 1, mpi_type_traits<FloatT>::type(), MPI_MAX, MPI_COMM_WORLD
        );
        MPI_Allreduce(
            &local_min_z, &global_min_z, 1, mpi_type_traits<FloatT>::type(), MPI_MIN, MPI_COMM_WORLD
        );
        MPI_Allreduce(
            &local_max_z, &global_max_z, 1, mpi_type_traits<FloatT>::type(), MPI_MAX, MPI_COMM_WORLD
        );
 ...
 *
 * Quantizes and encodes local particles, updates a local histogram array using GPU atomic
 * additions, and executes an MPI_Allreduce with MPI_SUM to compute the global particle count
 * distribution.
 *
 * @tparam FloatT Floating-point coordinate type.
 * @param[in] q SYCL queue.
 ...
 * @return std::vector<int> Global histogram containing total particle counts per coarse grid
 * bucket.
 */
template <typename FloatT>
inline std::vector<int> get_global_histogram(
    sycl::queue &q, const particles<FloatT> &p, const BoundingBox<FloatT> &global_bbox, int m = 20
) {
    size_t n           = p.pos_x.size();
    int    num_buckets = 1 << m;
    ...
    q.fill(d_local_hist, 0, num_buckets);
    ...
    if (n > 0) {
        ...
        q.parallel_for(sycl::range<1>(n), [=](sycl::id<1> idx) {
             ...
             auto     atomic_ref = sycl::atomic_ref<
                 ...
                 sycl::access::address_space::global_space>(d_local_hist[bucket_idx]);
    ...
    MPI_Allreduce(
        local_hist.data(), global_hist.data(), num_buckets, MPI_INT, MPI_SUM, MPI_COMM_WORLD
    );
...
generate_splitters(const std::vector<int> &global_hist, int P, int m = 20) {
    int       num_buckets     = 1 << m;
    ...
    // 2. Scan through buckets to assign splitter boundaries when cumulative load targets are
    // reached
 ...
 */
template <typename FloatT> inline void normalize_particles(particles<FloatT> &p, size_t n) {
 ...
 * and transmits them to Rank 0. Rank 0 aggregates and sorts these samples to determine boundaries
 * for P equal-load segments. These 64-bit key boundaries are then broadcast to all ranks.
 *
 * @tparam FloatT Floating-point coordinate type.
 * @param[in] q SYCL queue.
 * @param[in] p Local particles on this rank.
 ...
 */
template <typename FloatT>
inline std::vector<sfc_key> get_deterministic_splitters(
    sycl::queue &q, const particles<FloatT> &p, const BoundingBox<FloatT> &global_bbox
) {
    int rank, P;
 ...
 */
template <typename FloatT>
inline particles<FloatT> redistribute_particles(
    sycl::queue       &q,
    particles<FloatT> &p,
...
#endif
    const BoundingBox<FloatT> &global_bbox,
    int                        m = 20
) {
    int rank, P;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    ...
    if (n > 0) {
        ...
        q.copy(rank_splitters.data(), d_splitters, P + 1);

        // 5. Run binary search to identify target destination rank for each particle using full
        // sfc_key
        q.parallel_for(sycl::range<1>(n), [=](sycl::id<1> idx) {
             size_t i     = idx[0];
             d_indices[i] = i;
 ...
 * Received boundary particles are tagged with is_ghost = 1 and appended to the local particle
 * array.
 *
 * @tparam FloatT Floating-point coordinate type.
 * @param[in] q SYCL queue.
...
inline particles<FloatT> exchange_halos(sycl::queue &q, particles<FloatT> &p, FloatT h_max) {
    ...
    std::vector<FloatT> local_box_floats = {
        my_bbox.min_x, my_bbox.max_x, my_bbox.min_y, my_bbox.max_y, my_bbox.min_z, my_bbox.max_z
    };
    std::vector<FloatT> all_box_floats(P * 6);
    MPI_Allgather(
        local_box_floats.data(),
        6,
        mpi_type_traits<FloatT>::type(),
        all_box_floats.data(),
        6,
    ...
    std::vector<BoundingBox<FloatT>> all_bboxes(
        P,
        BoundingBox<FloatT>(
            static_cast<FloatT>(0.0),
            static_cast<FloatT>(0.0),
    ...
    for (int i = 0; i < P; ++i) {
        all_bboxes[i] = BoundingBox<FloatT>{
            all_box_floats[i * 6 + 0],
            all_box_floats[i * 6 + 1],
            all_box_floats[i * 6 + 2],
            all_box_floats[i * 6 + 3],
            all_box_floats[i * 6 + 4],
            all_box_floats[i * 6 + 5]
        };
    }

    // 4. Filter and identify direct spatial neighbors (overlapping within search radius h_max) on
    // CPU
    std::vector<int> neighbor_ranks;
    for (int r = 0; r < P; ++r) {
        if (r == rank) continue;
        if (all_bboxes[r].min_x > all_bboxes[r].max_x) continue; // empty rank
        if (my_bbox.min_x - h_max <= all_bboxes[r].max_x &&
            my_bbox.max_x + h_max >= all_bboxes[r].min_x &&
            my_bbox.min_y - h_max <= all_bboxes[r].max_y &&
            my_bbox.max_y + h_max >= all_bboxes[r].min_y &&
            my_bbox.min_z - h_max <= all_bboxes[r].max_z &&
            my_bbox.max_z + h_max >= all_bboxes[r].min_z) {
    ...
    // 6. Find local boundary particles overlapping with neighbor domains using a memory-efficient
    // two-pass approach
    if (n > 0 && !neighbor_ranks.empty()) {
        ...
        for (size_t k = 0; k < neighbor_ranks.size(); ++k) {
            ...
            q.parallel_for(sycl::range<1>(n), [=](sycl::id<1> idx) {
                 size_t i = idx[0];
                 if (dev_pos_x[i] >= min_x && dev_pos_x[i] <= max_x && dev_pos_y[i] >= min_y &&
                     dev_pos_y[i] <= max_y && dev_pos_z[i] >= min_z && dev_pos_z[i] <= max_z) {
                     auto atomic_ref = sycl::atomic_ref<
                         int,
                         sycl::memory_order::relaxed,
                         sycl::memory_scope::device,
                         sycl::access::address_space::global_space>(d_count[0]);
                     atomic_ref.fetch_add(1);
                 }
             }).wait();
