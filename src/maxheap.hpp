 ...
 */
template <typename DistT, typename IdxT>
inline void local_heap_sift_up(
    const sycl::local_accessor<DistT, 1> &sh_dist,
    const sycl::local_accessor<IdxT, 1>  &sh_idx,
    int                                   i
) noexcept {
 ...
 */
template <typename DistT, typename IdxT>
inline void local_heap_sift_down(
    const sycl::local_accessor<DistT, 1> &sh_dist,
    const sycl::local_accessor<IdxT, 1>  &sh_idx,
    int                                   i,
    int                                   size
 ...
 */
template <typename DistT, typename IdxT> struct SharedMaxHeap {
    static_assert(
        std::is_floating_point_v<DistT> || std::is_integral_v<DistT>,
    ...
    static bool should_prune(
        DistT                                 node_min_d2,
        const sycl::local_accessor<DistT, 1> &sh_worst,
    ...
    /**
     * @brief Performs parallel bitonic sort over shared heap and writes top-k sorted results
     * ascending.
     */
    static void extract_sorted_k(
        sycl::nd_item<1>                     &item,
        const sycl::local_accessor<DistT, 1> &sh_dist,
        const sycl::local_accessor<IdxT, 1>  &sh_idx,
        const sycl::local_accessor<int, 1>   &sh_count,
